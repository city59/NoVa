...
# # # #---------IJCAI--------------------------------------------------------------------------------------------------------------
	...
	parser.add_argument("--n_hops", type=int, default=3, help='Num layers of GNN')
	parser.add_argument("--noise_eps", type=float, default=0.1, help='noise_eps')
	parser.add_argument("--cl_tau", type=float, default=0.3, help='InfoNCE tau')
	parser.add_argument("--cl_rate", type=float, default=0.05, help='cl rate for InfoNCE')
	parser.add_argument("--tail_k_threshold", type=int, default=5, help='购买行为的次数阈值')
	parser.add_argument("--hesitation_threshold", type=float, default=0.6, help='犹豫集阈值')
	parser.add_argument("--hesitation_weight", type=float, default=0.5 ,help='犹豫集损失权重')
	parser.add_argument("--m_h_eps", type=float, default=0.001, help='知识迁移模块损失权重')
	parser.add_argument("--kl_eps", type=float, default=10.0, help='KL散度损失权重')
	parser.add_argument("--l2_reg", type=float, default=0.01, help='L2正则化系数')
	parser.add_argument("--neighbor_norm_type", type=str,default='both', help='neighbor_norm_type for LAGCL')
	parser.add_argument("--agg_function", type=int, default=0, help='聚合函数类型')
	parser.add_argument("--annealing_type", type=int, default=1, help='退火类型')
	parser.add_argument("--lambda_gp", type=float, default=0.1, help='梯度惩罚系数')
	parser.add_argument('--use_relation_rf', type=bool, default=True, help='是否使用关系预测模块')
	parser.add_argument("--agg_w_exp_scale", type=float, default=20, help='聚合权重指数缩放')
	parser.add_argument("--lagcl_lr",type=float,default=0.001, help='learning_rate')
	parser.add_argument("--metric_topk", type=int, default=20, help='metric_topk')
...
# 	parser.add_argument('--hidden_dim', default=16, type=int, help='embedding size')
# 	parser.add_argument('--gnn_layer', default="[16,16,16]", type=str, help='gnn layers: number + dim')
# 	parser.add_argument('--dataset', default='Retail', type=str, help='name of dataset')
# 	parser.add_argument('--point', default='for_meta_hidden_dim', type=str, help='')
# 	parser.add_argument('--title', default='dim__8', type=str, help='title of model')
# 	parser.add_argument('--sampNum', default=40, type=int, help='batch size for sampling')
#
