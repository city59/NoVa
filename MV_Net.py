...
#IJCAI,Tmall--------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            # Reshape to (N, 1) before batch norm
            if behavior_loss_multi_list_weights[i].dim() == 0:
                behavior_loss_multi_list_weights[i] = behavior_loss_multi_list_weights[i].unsqueeze(0)

            # Add check for batch size before batch norm
            if behavior_loss_multi_list_weights[i].size(0) > 1:
                behavior_loss_multi_list_weights[i] = self.batch_norm(behavior_loss_multi_list_weights[i].unsqueeze(1))
            else:
                behavior_loss_multi_list_weights[i] = behavior_loss_multi_list_weights[i].unsqueeze(1)

            behavior_loss_multi_list_weights[i] = args.inner_product_mult*self.sigmoid(behavior_loss_multi_list_weights[i]).squeeze()
            RS_weight3 = self.dropout7(self.prelu(self.RS_layer3(RS_input3)))

            if RS_weight3.dim() == 0:
                RS_weight3 = RS_weight3.unsqueeze(0)

            # Add check for batch size before batch norm
            if RS_weight3.size(0) > 1:
                RS_weight3 = self.batch_norm(RS_weight3.unsqueeze(1)).squeeze()
            else:
                RS_weight3 = RS_weight3.unsqueeze(1).squeeze()
