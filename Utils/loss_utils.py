import torch
import torch.nn.functional as F
import torch.nn as nn


def bpr_loss(user_emb, pos_item_emb, neg_item_emb, low_pos_item_emb, reduction='mean'):
    pos_score = torch.mul(user_emb, pos_item_emb).sum(dim=1)
    neg_score = torch.mul(user_emb, neg_item_emb).sum(dim=1)

    if low_pos_item_emb is not None:
        low_pos_score = torch.mul(user_emb, low_pos_item_emb).sum(dim=1)
        loss = -torch.log(1e-10 + torch.sigmoid(pos_score - neg_score)) + \
               -torch.log(1e-10 + torch.sigmoid(low_pos_score - neg_score))
    else:
        loss = -torch.log(1e-10 + torch.sigmoid(pos_score - neg_score))

    if reduction == 'mean':
        loss = torch.mean(loss)

    if reduction == 'sum':
        reduce_fun = torch.sum
    elif reduction == 'mean':
        reduce_fun = torch.mean
    else:
        reduce_fun = lambda x: x
    return reduce_fun(loss)


def l2_reg_loss(reg, *args):
    emb_loss = 0
    for emb in args:
        emb_loss += torch.norm(emb, p=2)
    return emb_loss * reg


def InfoNCE(view1, view2, temperature, view_all=None):
    """
    view1: 视图 1
    view2: 视图 2
    view_all: 视图 2 的所有节点表征，留空则使用视图 2
    """
    view1 = F.normalize(view1, dim=1)
    view2 = F.normalize(view2, dim=1)
    if view_all is None:
        view_all = view2
    else:
        view_all = F.normalize(view_all, dim=1)
    pos_score = (view1 * view2).sum(dim=-1)
    pos_score = torch.exp(pos_score / temperature)
    ttl_score = torch.matmul(view1, view_all.transpose(0, 1))
    ttl_score = torch.exp(ttl_score / temperature).sum(dim=1)
    cl_loss = -torch.log(pos_score / ttl_score)
    return torch.mean(cl_loss)
